...
impl<'a> Node<'a> {
    pub fn new(depth: usize, index: usize, parent: *const Node<'a>) -> Self {
        Node {
            in_dict: false,
            depth: depth,
            index: index,
...
impl<'a> Automaton<'a> {
    ...
    fn insert(node: &mut Node, word: &str) {
        ...
        let child: &mut Node = node.next[index]
            .get_or_insert(Box::new(Node::new(node.depth + 1, index, raw_node)))
            .borrow_mut();
...
fn main() {
    ...
    for hit in hits.iter() {
        let entry = stats.entry(&text[hit.0..hit.1]).or_insert(Vec::new());
        entry.push(hit.0);
    }
